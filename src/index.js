...
async function callGroq(messages, groqKey, stream) {
  ...
  if (!res.ok) {
    // Don't throw — return failed so we can fallback
...
export default {
  async fetch(request, env) {
    ...
    if (wantsStream) {
            ...
            while (true) {
              ...
              for (const line of lines) {
                if (line.startsWith("data: ")) {
                  const data = line.slice(6).trim();
                  if (data === "[DONE]") {
                    const doneChunk = { id: id, object: "chat.completion.chunk", created: created, model: MODEL_NAME, choices: [{ index: 0, delta: {}, finish_reason: "stop" }] };
      ...
      return new Response(JSON.stringify({ error: { message: "All providers failed", type: "ai_error" } }), { status: 500, headers: jsonHeaders });
    }
    ...
    if (groqResult.ok && groqResult.content !== undefined) {
      ...
      if (url.pathname === "/api/chat") {
        return new Response(JSON.stringify({ model: MODEL_NAME, created_at: new Date().toISOString(), message: { role: "assistant", content: content }, done_reason: "stop", done: true, total_duration: 1000000000, load_duration: 0, prompt_eval_count: 0, prompt_eval_duration: 0, eval_count: 0, eval_duration: 0 }), { headers: jsonHeaders });
      }
      if (url.pathname === "/api/generate") {
        return new Response(JSON.stringify({ model: MODEL_NAME, created_at: new Date().toISOString(), response: content, done: true, done_reason: "stop" }), { headers: jsonHeaders });
      }
      return new Response(JSON.stringify({ id: id, object: "chat.completion", created: created, model: MODEL_NAME, choices: [{ index: 0, message: { role: "assistant", content: content }, finish_reason: "stop" }], usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }), { headers: jsonHeaders });
    }

    // Cloudflare fallback
    console.log("Groq failed:", groqResult.error, "— using Cloudflare");
    ...
    return new Response(JSON.stringify({ error: { message: cfResult.error || "All providers failed", type: "ai_error" } }), { status: 500, headers: jsonHeaders });
  }
};
